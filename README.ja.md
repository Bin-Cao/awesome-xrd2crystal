...
> [!TIP]
> ## ✨ インタラクティブな文献マップを探索
> **[検索可能な XRD-to-Crystal Explorer を開く →](https://bin-cao.github.io/awesome-xrd2crystal/)**
>
> このサイトは、リポジトリ内の [`index.html`](index.html) をインタラクティブ化したものです。全エントリを検索し、研究分野または年で絞り込めます。
...
PXRD から構造を推定する問題は不適切設定の逆問題です。論文タイトル、著者、所属、技術的な要約で誤訳を避けるため、文献エントリは英語のまま維持しています。
...
このリストでは、主に二つの相補的な問題設定が使われます。目的に最も近いセクションを選んでください：

| タスク | 入力 | 出力 | セクション |
|---|---|---|---|
| **相 / 構造同定**（分類、検索） | PXRD パターン | 既知相、結晶系、空間群のインデックス | [相同定](#phase-identification-from-xrd) · [結晶系 / 空間群 / 格子](#crystal-system--space-group--lattice-prediction-from-xrd) |
| **多相分解** | 混合 PXRD パターン | 各相の重量分率と成分パターン | [多相分解と Rietveld](#multi-phase-decomposition--rietveld-refinement) |
| **ab-initio 構造生成** | PXRD パターン（必要に応じて組成 / セル） | 完全な 3D 結晶（原子種 + 分数座標 + 格子 / CIF） | [End-to-End PXRD から結晶構造へ（生成）](#end-to-end-pxrd--crystal-structure-generative) |
| **自律 / in-situ 解析** | ビームライン / ラボのライブ PXRD ストリーム | 相判定、次測定方針、合成判断 | [自律 / in-situ XRD](#autonomous--in-situ-xrd-analysis) |

分類モデルは通常より軽量で、対称性等変性を必須とせず、分布外パターンでも比較的なだらかに性能が低下します。一方、生成モデルは原子分解能の構造を目標としますが、より強い物理事前知識に依存し、訓練データのカバレッジにも制約されます。

---

## 目次

- [概要](#overview-localized)
- [メンテナー](#maintainers)
- [分類と生成：クイックガイド](#classification-vs-generation--quick-guide)
- [標準エントリ形式](#standard-entry-format)
- [End-to-End PXRD から結晶構造へ（生成）](#end-to-end-pxrd--crystal-structure-generative)
- [XRD による相同定](#phase-identification-from-xrd)
- [XRD からの結晶系 / 空間群 / 格子予測](#crystal-system--space-group--lattice-prediction-from-xrd)
- [多相分解と Rietveld 精密化](#multi-phase-decomposition--rietveld-refinement)
- [自律 / in-situ XRD 解析](#autonomous--in-situ-xrd-analysis)
- [データセットとベンチマーク](#datasets--benchmarks)
- [シミュレーション、精密化、ユーティリティツール](#simulation-refinement-and-utility-tools)
- [レビューとサーベイ](#reviews--surveys)
- [関連キュレーションリスト](#related-curated-lists)
...
## End-to-End PXRD から結晶構造へ（生成）

PXRD パターン（必要に応じて組成 / 格子情報）を入力し、完全な 3D 結晶構造（原子位置 + 格子、通常は CIF または座標）を出力する方法を収録しています。
...
PXRD パターンから対応する既知相を予測する方法（分類 / マルチラベル / 一対一同定）。
...
時系列で掲載しています。*参照データベース（RRUFF、COD、Materials Project、ICSD、JARVIS-DFT）は末尾にまとめています。*
...
- **[Issue を作成](https://github.com/Bin-Cao/awesome-xrd2crystal/issues/new)** — 論文タイトル、arXiv ID、GitHub リンク、可能であれば該当セクションを記載してください。
- **Pull Request を作成** — [CONTRIBUTING.md](CONTRIBUTING.md) の形式に従って README を編集してください。
- **メンテナーにメール** — まとまった追加や未掲載の研究方向がある場合に適しています。

完全なエントリ形式、時系列ルール、リンク規約は [CONTRIBUTING.md](CONTRIBUTING.md) を参照してください。
...
<sub>*アバターは GitHub contributor graph から [contrib.rocks](https://contrib.rocks) により自動生成されます。*</sub>
