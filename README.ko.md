...
PXRD로부터 구조를 추정하는 문제는 ill-posed inverse problem입니다. 논문 제목, 저자, 소속, 기술 요약의 오역을 줄이기 위해 문헌 항목은 영어 원문을 유지합니다.
...
이 목록에서는 두 가지 상호 보완적인 문제 설정이 반복적으로 사용됩니다. 목적에 가장 가까운 섹션을 선택하세요:

| 작업 | 입력 | 출력 | 섹션 |
|---|---|---|---|
| **상 / 구조 식별**(분류, 검색) | PXRD 패턴 | 알려진 상, 결정계 또는 공간군 인덱스 | [상 식별](#phase-identification-from-xrd) · [결정계 / 공간군 / 격자](#crystal-system--space-group--lattice-prediction-from-xrd) |
| **다상 분해** | 혼합 PXRD 패턴 | 각 상의 중량 분율 및 성분 패턴 | [다상 분해 및 Rietveld](#multi-phase-decomposition--rietveld-refinement) |
| **ab-initio 구조 생성** | PXRD 패턴(조성 / 셀 선택 포함) | 완전한 3D 결정(원자종 + 분수 좌표 + 격자 / CIF) | [End-to-End PXRD에서 결정 구조로(생성형)](#end-to-end-pxrd--crystal-structure-generative) |
| **자율 / in-situ 분석** | 빔라인 / 실험실의 실시간 PXRD 스트림 | 상 판정, 다음 측정 정책, 합성 의사결정 | [자율 / in-situ XRD](#autonomous--in-situ-xrd-analysis) |

분류 모델은 일반적으로 더 가볍고 대칭 등변성을 반드시 요구하지 않으며, 분포 밖 패턴에서 비교적 완만하게 성능이 저하됩니다. 생성 모델은 원자 해상도 구조를 목표로 하지만 더 강한 물리 사전지식에 의존하고 훈련 데이터 범위의 제약을 받습니다.

---

## 목차

- [개요](#overview-localized)
- [관리자](#maintainers)
- [분류와 생성: 빠른 가이드](#classification-vs-generation--quick-guide)
- [표준 항목 형식](#standard-entry-format)
- [End-to-End PXRD에서 결정 구조로(생성형)](#end-to-end-pxrd--crystal-structure-generative)
- [XRD 기반 상 식별](#phase-identification-from-xrd)
- [XRD 기반 결정계 / 공간군 / 격자 예측](#crystal-system--space-group--lattice-prediction-from-xrd)
- [다상 분해 및 Rietveld 정련](#multi-phase-decomposition--rietveld-refinement)
- [자율 / in-situ XRD 분석](#autonomous--in-situ-xrd-analysis)
- [데이터셋 및 벤치마크](#datasets--benchmarks)
- [시뮬레이션, 정련, 유틸리티 도구](#simulation-refinement-and-utility-tools)
- [리뷰 및 서베이](#reviews--surveys)
- [관련 큐레이션 목록](#related-curated-lists)
- [기여](#contributing)
- [기여자](#contributors)
...
## End-to-End PXRD에서 결정 구조로(생성형)

PXRD 패턴(선택적으로 조성 / 격자 정보 포함)을 입력으로 받아 완전한 3D 결정 구조(원자 위치 + 격자, 일반적으로 CIF 또는 좌표)를 출력하는 방법을 수록합니다.
...
PXRD 패턴이 어떤 알려진 상에 대응하는지 예측하는 방법(분류 / 다중 라벨 / 일대일 식별).
...
시간순으로 정리했습니다. *참조 데이터베이스(RRUFF, COD, Materials Project, ICSD, JARVIS-DFT)는 참고용으로 마지막에 모았습니다.*
...
PXRD 기반 분석과 구조 결정에 직접 초점을 둔 리뷰:
...
더 넓은 결정 CSP / 생성 모델 리뷰(위 방법들의 배경):
...
- **[Issue 열기](https://github.com/Bin-Cao/awesome-xrd2crystal/issues/new)** — 논문 제목, arXiv ID 또는 GitHub 링크, 가능하면 해당 섹션을 적어 주세요.
- **Pull Request 열기** — [CONTRIBUTING.md](CONTRIBUTING.md)의 형식에 맞춰 README를 수정해 주세요.
- **관리자에게 이메일 보내기** — 누락된 연구 방향 등 대량 추가에 적합합니다.

전체 항목 템플릿, 시간순 규칙, 링크 규칙은 [CONTRIBUTING.md](CONTRIBUTING.md)를 참고하세요.
...
논문 추가, 수정, 개선에 기여해 주신 분들께 감사드립니다:
...
<sub>*아바타는 GitHub contributor graph를 기반으로 [contrib.rocks](https://contrib.rocks)가 자동 생성합니다.*</sub>
