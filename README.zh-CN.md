...
> [!TIP]
> ## ✨ 在线交互式文献地图
> **[打开可搜索的 XRD-to-Crystal Explorer →](https://bin-cao.github.io/awesome-xrd2crystal/)**
>
> 此在线页面即为本仓库 [`index.html`](index.html) 的交互式版本，可搜索全部条目，并按研究方向或年份筛选。
...
PXRD 到结构是典型的病态反问题：不同结构可能产生相似的一维衍射图谱，而实验数据还会受到仪器展宽、晶粒尺寸、择优取向、背景、杂相和峰重叠等因素影响。文献条目保持英文，以避免论文标题、作者、机构和技术摘要在翻译中产生歧义。
...
本列表反复出现两类互补的问题设定。请选择与你任务最接近的章节：

| 任务 | 输入 | 输出 | 相关章节 |
|---|---|---|---|
| **物相 / 结构识别**（分类、检索） | PXRD 图谱 | 已知物相、晶系或空间群索引 | [物相识别](#phase-identification-from-xrd) · [晶系 / 空间群 / 晶格](#crystal-system--space-group--lattice-prediction-from-xrd) |
| **多相分解** | 混合 PXRD 图谱 | 各相含量与组分图谱 | [多相分解与 Rietveld](#multi-phase-decomposition--rietveld-refinement) |
| **从头结构生成** | PXRD 图谱（可选成分 / 晶胞） | 完整三维晶体（原子种类 + 分数坐标 + 晶格 / CIF） | [端到端 PXRD 到晶体结构（生成式）](#end-to-end-pxrd--crystal-structure-generative) |
| **自主 / 原位分析** | 实验室或束线实时 PXRD 数据流 | 物相判断、下一步测量策略、合成决策 | [自主 / 原位 XRD](#autonomous--in-situ-xrd-analysis) |

分类模型通常更轻量，不一定需要对称等变性，并且在分布外图谱上退化更平滑；生成模型面向原子级结构解析，但依赖更强的物理先验，并仍受训练集覆盖范围约束。

---

## 目录

- [项目概览](#overview-localized)
- [维护者](#maintainers)
- [分类与生成：快速指南](#classification-vs-generation--quick-guide)
- [标准条目格式](#standard-entry-format)
- [端到端 PXRD 到晶体结构（生成式）](#end-to-end-pxrd--crystal-structure-generative)
- [XRD 物相识别](#phase-identification-from-xrd)
- [从 XRD 预测晶系 / 空间群 / 晶格](#crystal-system--space-group--lattice-prediction-from-xrd)
- [多相分解与 Rietveld 精修](#multi-phase-decomposition--rietveld-refinement)
- [自主 / 原位 XRD 分析](#autonomous--in-situ-xrd-analysis)
- [数据集与基准](#datasets--benchmarks)
- [模拟、精修与实用工具](#simulation-refinement-and-utility-tools)
- [综述与调研](#reviews--surveys)
- [相关精选列表](#related-curated-lists)
- [贡献](#contributing)
- [贡献者](#contributors)
...
本节收录以 PXRD 图谱（可选成分 / 晶格信息）为输入，并输出完整三维晶体结构（原子位置 + 晶格，通常为 CIF 或坐标）的生成式方法。
...
给定 PXRD 图谱，预测其对应的已知物相（分类 / 多标签 / 一对一识别）的方法。
...
按时间顺序列出。*常用参考数据库（RRUFF、COD、Materials Project、ICSD、JARVIS-DFT）统一放在本节末尾。*
...
- **[提交 issue](https://github.com/Bin-Cao/awesome-xrd2crystal/issues/new)** — 粘贴论文标题、arXiv 编号或 GitHub 链接，并尽量说明所属章节。
- **提交 Pull Request** — 按 [CONTRIBUTING.md](CONTRIBUTING.md) 中的格式编辑 README。
- **邮件联系维护者** — 适合批量补充，例如新增一个遗漏的研究方向。

完整条目模板、时间顺序规则和链接约定请见 [CONTRIBUTING.md](CONTRIBUTING.md)。
...
<sub>*头像由 [contrib.rocks](https://contrib.rocks) 基于 GitHub contributor graph 自动生成。*</sub>
