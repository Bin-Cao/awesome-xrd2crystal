...
**Eine kuratierte Wissenskarte für PXRD-basierte Kristallidentifikation, Generierung, Zerlegung und Verfeinerung.**
...
<a id="overview-localized"></a>
## Überblick

Dieses Repository ist eine chronologisch kuratierte Sammlung von Papers, Code, Datensätzen, Benchmarks und Tools zu **XRD/PXRD zu Kristallstruktur**. Es umfasst Phasenidentifikation, Vorhersage von Kristallsystem, Raumgruppe und Gitterparametern, Mehrphasenzerlegung, Rietveld-Automatisierung, generative Strukturaufklärung sowie autonome und in-situ Analyse.

Die Bestimmung einer Struktur aus PXRD-Daten ist ein schlecht gestelltes inverses Problem. Um Übersetzungsfehler bei Titeln, Autoren, Institutionen und technischen Zusammenfassungen zu vermeiden, bleiben die Literatureinträge auf Englisch.
...
<a id="classification-vs-generation--quick-guide"></a>
## Klassifikation und Generierung: Kurzleitfaden

Zwei komplementäre Problemformulierungen tauchen in dieser Liste immer wieder auf. Wähle den Abschnitt, der am besten zu deiner Aufgabe passt:

| Aufgabe | Eingabe | Ausgabe | Abschnitte |
|---|---|---|---|
| **Phasen- / Strukturidentifikation** (Klassifikation, Retrieval) | PXRD-Muster | Index einer bekannten Phase, eines Kristallsystems oder einer Raumgruppe | [Phasenidentifikation](#phase-identification-from-xrd) · [Kristallsystem / Raumgruppe / Gitter](#crystal-system--space-group--lattice-prediction-from-xrd) |
| **Mehrphasenzerlegung** | Gemischtes PXRD-Muster | Gewichtsanteile und Komponenten-Muster pro Phase | [Mehrphasenzerlegung und Rietveld](#multi-phase-decomposition--rietveld-refinement) |
| **Ab-initio-Strukturgenerierung** | PXRD-Muster (± Zusammensetzung / Zelle) | Vollständiger 3D-Kristall (Atomsorten + fraktionale Koordinaten + Gitter / CIF) | [End-to-End PXRD zu Kristallstruktur (Generativ)](#end-to-end-pxrd--crystal-structure-generative) |
| **Autonome / in-situ Analyse** | Live-PXRD-Datenstrom aus Beamline / Labor | Phasenzuordnung, nächste Messstrategie, Syntheseentscheidungen | [Autonome / In-situ XRD](#autonomous--in-situ-xrd-analysis) |

Klassifikationsmodelle sind meist leichter, benötigen nicht zwingend Symmetrie-Äquivarianz und degradieren bei Out-of-Distribution-Mustern oft robuster. Generative Modelle zielen auf atomar aufgelöste Strukturen, hängen aber stärker von physikalischen Priors ab und bleiben durch die Abdeckung der Trainingsdaten begrenzt.

---

## Inhalt

- [Überblick](#overview-localized)
- [Maintainer](#maintainers)
- [Klassifikation und Generierung: Kurzleitfaden](#classification-vs-generation--quick-guide)
- [Standardformat für Einträge](#standard-entry-format)
- [End-to-End PXRD zu Kristallstruktur (Generativ)](#end-to-end-pxrd--crystal-structure-generative)
- [Phasenidentifikation aus XRD](#phase-identification-from-xrd)
- [Vorhersage von Kristallsystem / Raumgruppe / Gitter aus XRD](#crystal-system--space-group--lattice-prediction-from-xrd)
- [Mehrphasenzerlegung und Rietveld-Verfeinerung](#multi-phase-decomposition--rietveld-refinement)
- [Autonome / In-situ XRD-Analyse](#autonomous--in-situ-xrd-analysis)
- [Datensätze und Benchmarks](#datasets--benchmarks)
- [Simulations-, Verfeinerungs- und Hilfswerkzeuge](#simulation-refinement-and-utility-tools)
- [Reviews und Surveys](#reviews--surveys)
- [Verwandte kuratierte Listen](#related-curated-lists)
- [Beitragen](#contributing)
- [Mitwirkende](#contributors)

<a id="standard-entry-format"></a>
## Standardformat für Einträge
...
<a id="end-to-end-pxrd--crystal-structure-generative"></a>
## End-to-End PXRD zu Kristallstruktur (Generativ)

Methoden, die ein PXRD-Muster (optional mit Zusammensetzung / Gitter) als Eingabe nehmen und eine vollständige 3D-Kristallstruktur ausgeben (Atompositionen + Gitter, typischerweise als CIF oder Koordinaten).
...
<a id="phase-identification-from-xrd"></a>
## Phasenidentifikation aus XRD

Methoden, die aus einem PXRD-Muster vorhersagen, welchen bekannten Phasen es entspricht (Klassifikation / Multi-Label / One-to-One-ID).
...
<a id="crystal-system--space-group--lattice-prediction-from-xrd"></a>
## Vorhersage von Kristallsystem / Raumgruppe / Gitter aus XRD
...
<a id="multi-phase-decomposition--rietveld-refinement"></a>
## Mehrphasenzerlegung und Rietveld-Verfeinerung
...
<a id="autonomous--in-situ-xrd-analysis"></a>
## Autonome / In-situ XRD-Analyse
...
Chronologisch gelistet. *(Referenzdatenbanken wie RRUFF, COD, Materials Project, ICSD und JARVIS-DFT sind am Ende zusammengefasst.)*
...
<a id="reference-structure-databases-used-by-virtually-every-entry-above"></a>
### Referenzdatenbanken für Strukturen
...
<a id="simulation-refinement-and-utility-tools"></a>
## Simulations-, Verfeinerungs- und Hilfswerkzeuge
...
<a id="reviews--surveys"></a>
## Reviews und Surveys

Reviews mit direktem Fokus auf PXRD-gestützte Analyse und Strukturbestimmung:
...
Breitere Reviews zu Crystal-CSP und generativen Modellen (Kontext für die obigen Methoden):
...
<a id="related-curated-lists"></a>
## Verwandte kuratierte Listen
...
**Ergänzungen, Korrekturen und Übersetzungen sind willkommen.**

Es gibt drei einfache Möglichkeiten beizutragen:

- **[Issue öffnen](https://github.com/Bin-Cao/awesome-xrd2crystal/issues/new)** — Paper-Titel, arXiv-ID oder GitHub-Link einfügen und möglichst den passenden Abschnitt nennen.
- **Pull Request öffnen** — README gemäß dem Format in [CONTRIBUTING.md](CONTRIBUTING.md) bearbeiten.
- **Maintainer per E-Mail kontaktieren** — geeignet für größere Ergänzungen, z. B. eine fehlende Forschungsrichtung.

Die vollständige Vorlage, Chronologieregeln und Link-Konventionen stehen in [CONTRIBUTING.md](CONTRIBUTING.md).
...
Vielen Dank an alle, die Papers, Korrekturen und Verbesserungen zu dieser Liste beigetragen haben:
...
<sub>*Avatare werden automatisch von [contrib.rocks](https://contrib.rocks) aus dem GitHub Contributor Graph erzeugt.*</sub>
